...
typedef struct {
	...
	void* data; 
	// Only first buffer is used for samples
	ALuint buffers[2];
	float volume;
	bool active;
} Sound;
...
SoundHandle sound_load_sample(const char* filename) {
	...
	// Data isn't needed, everything is stored in openal buffer
	sounds[result].data = NULL;
...
void sound_free(SoundHandle handle) {
	...
	if(sounds[handle].is_stream)
		LOG_ERROR("Streams are not supported yet");
	...
	// Delete buffer
	alDeleteBuffers(1, &(sounds[handle].buffers[0]));
...
void sound_play(SoundHandle handle) {
	...
	if(sounds[handle].is_stream)
		 LOG_ERROR("Sounds streams not implemented yet");

	// Find free source
	uint source_id;
	for(source_id = 0; source_id < MAX_SOURCES; ++source_id) {
		ALint state;
